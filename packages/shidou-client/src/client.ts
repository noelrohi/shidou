...
export interface ShidouClientOptions {
...
/** Browser-safe client for Shidou's versioned JSON-over-WebSocket protocol. */
export class ShidouClient {
  ...
  constructor(options: ShidouClientOptions) {
  ...
  connect(): Promise<void> {
    ...
    if (this.state === "connecting") {
      return Promise.reject(new Error("Shidou client is already connecting"));
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      socket.addEventListener("message", (event) => {
        ...
        try {
          ...
          failHandshake(new Error("Shidou daemon sent invalid JSON"));
          return;
        }
        ...
        if (!handshakeSettled) {
          ...
          failHandshake(new Error("Shidou daemon sent an invalid handshake response"));
      ...
      socket.addEventListener("error", () => {
        failHandshake(new Error("Shidou daemon connection failed"));
      });
      socket.addEventListener("close", () => {
        if (generation !== this.connectionGeneration) return;
        failHandshake(new Error("Shidou daemon disconnected during handshake"));
        this.markDisconnected(new Error("Shidou daemon disconnected"));
      });
    });
  }
  ...
  request(
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(new Error("timed out waiting for Shidou daemon"));
  ...
  disconnect(): void {
    this.rejectConnect?.(new Error("Shidou client disconnected"));
    ++this.connectionGeneration;
    const socket = this.socket;
    this.socket = undefined;
    this.markDisconnected(new Error("Shidou client disconnected"));
  ...
  private requireSocket(): WebSocketLike {
    if (this.state !== "connected" || !this.socket || this.socket.readyState !== OPEN) {
      throw new Error("Shidou daemon is disconnected");
    }
    return this.socket;
  }
