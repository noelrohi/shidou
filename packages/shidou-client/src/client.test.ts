...
import { ShidouClient, ShidouRpcError, daemonUrl, type WebSocketLike } from "./client";
...
async function connect(client: ShidouClient, sockets: FakeSocket[]): Promise<FakeSocket> {
...
describe("ShidouClient", () => {
  ...
  test("disconnect rejects an in-flight handshake and permits reconnecting", async () => {
    ...
    const firstConnection = client.connect();
    client.disconnect();
    await expect(firstConnection).rejects.toThrow("Shidou client disconnected");
  ...
  test("disconnected requests reject instead of throwing synchronously", async () => {
    ...
    await expect(request).rejects.toThrow("Shidou daemon is disconnected");
  });
  ...
  test("disconnected notifications reject instead of throwing synchronously", async () => {
    ...
    await expect(notification).rejects.toThrow("Shidou daemon is disconnected");
  });
...
test("daemonUrl pins the versioned endpoint", () => {
  ...
  expect(daemonUrl("wss://shidou.example.test")).toBe("wss://shidou.example.test/v1");
});
