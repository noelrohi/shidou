...
//
// <assets-dir> holds this release's `Shidou-<version>-<arch>-Setup.exe` files.
...
<rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" version="2.0">
  <channel>
    <title>Shidou (Windows ${arch})</title>
${entries}
  </channel>
</rss>
`;
...
export async function generateWindowsAppcasts(
  ...
  for (const arch of architectures) {
    const installer = `Shidou-${version}-${arch}-Setup.exe`;
  ...
  if (written.length === 0) {
    throw new Error(`No Shidou-${version}-<arch>-Setup.exe found in ${assetsDir}`);
  }
  return written;
}
