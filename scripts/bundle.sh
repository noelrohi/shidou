...
debug_identity_cache=".shidou-cache/codesign/debug-identity"
codesign_identity_from_environment=0
if [ -n "${SHIDOU_CODESIGN_IDENTITY:-}" ]; then
  codesign_identity="$SHIDOU_CODESIGN_IDENTITY"
...
case "$profile" in
  debug)
    app_name="Shidou Debug"
    helper_name="Shidou Debug Computer Use"
    bundle_identifier="dev.shidou.debug"
    icon_file="AppIconDev.icns"
    ;;
  release)
    app_name="Shidou"
    helper_name="Shidou Computer Use"
    bundle_identifier="dev.shidou"
...
helper_cache_root=".shidou-cache/computer-use/$profile"
...
# The cached app is copied into Shidou's standard Helpers directory as the
# canonical packaged service. Shidou refreshes a stable standalone runtime copy
# from it so Screen Recording is attributed to the helper rather than Shidou.
...
sparkle_cache_root=".shidou-cache/sparkle"
...
# Shidou is not sandboxed, so Sparkle's XPC services never run; drop them along
