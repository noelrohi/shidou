import type { AgentSession, ProviderKind, ProviderModel, ProviderProbe } from '@shidou/client'
import { Popover } from '@base-ui/react/popover'
import { useEffect, useRef, useState, type RefObject } from 'react'
import { ProviderIcon, PROVIDERS, providerMeta, ShidouIcon } from '@/components/shidou-icon'
...
export function ModelPicker({
  ...
  const [favorites, setFavorites] = useState<string[]>(() => {
    ...
    try { return JSON.parse(window.localStorage.getItem('shidou.favorite-models') ?? '[]') as string[] }
    catch { return [] }
  })
  ...
  function toggleFavorite(provider: ProviderKind, model: string) {
    ...
    setFavorites((current) => {
      ...
      window.localStorage.setItem('shidou.favorite-models', JSON.stringify(next))
      return next
    })
  }
    ...
    <Popover.Root modal={false} open={open} onOpenChange={setOpen}>
      ...
      <Popover.Portal>
        ...
        >
          <Popover.Popup
            aria-label={t('models.choose')}
            className="shidou-popover-surface flex h-[390px] w-[460px] max-w-[calc(100vw-32px)] overflow-hidden rounded-[12px] outline-none"
          ...
          <div className="flex h-full w-[50px] shrink-0 flex-col items-center gap-1 overflow-y-auto border-r bg-background p-[5px]">
            <ModelTab active={tab === 'favorites' && !query} label={t('models.favorites')} onClick={() => { setTab('favorites'); setQuery(''); setHighlight(null) }}>
              <ShidouIcon className="size-[17px]" name="star" />
            </ModelTab>
              ...
              {rows.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <ShidouIcon className={cn('size-3.5 text-[var(--text-ghost)]', favorite && 'text-amber-500')} name={favorite ? 'starFilled' : 'star'} />
                    </span>
                  </div>
                )
              })}
