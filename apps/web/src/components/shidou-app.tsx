...
export function ShidouApp() {
  ...
  useEffect(() => {
    window.localStorage.setItem('shidou.sidebarVisible', String(sidebarVisible))
  }, [sidebarVisible])

  useEffect(() => {
    const timer = window.setTimeout(() => {
      window.localStorage.setItem('shidou.sidebarWidth', String(Math.round(sidebarWidth)))
    }, 150)
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      window.localStorage.setItem('shidou.rightPanelWidth', String(Math.round(rightPanelWidth)))
    }, 150)
    return () => window.clearTimeout(timer)
  }, [rightPanelWidth])
    ...
    <div className="flex h-dvh w-full overflow-hidden bg-background">
      ...
      <main className="relative flex min-w-0 flex-1 flex-col">
        <TaskHeader
          ...
          session={activeSession}
          sidebarVisible={sidebarVisible}
          title={newTaskMode ? t('menu.new_task') : current ? displayTitle(current) : 'Shidou'}
        />
...
function readSidebarWidth(): number {
  ...
  const raw = window.localStorage.getItem('shidou.sidebarWidth')
...
function readSidebarVisible(): boolean {
  ...
  return window.localStorage.getItem('shidou.sidebarVisible') !== 'false'
}

function readRightPanelWidth(): number {
  if (typeof window === 'undefined') return 460
  const raw = window.localStorage.getItem('shidou.rightPanelWidth')
      ...
      <Popover.Portal>
        ...
        >
          <Popover.Popup
            ...
            className="shidou-popover-surface max-h-[420px] w-[min(300px,calc(100vw-24px))] overflow-y-auto rounded-[12px] p-2 text-popover-foreground outline-none"
            initialFocus={false}
            ref={popup}
            role="menu"
            onKeyDown={(event) => {
              ...
              {(item.key.kind !== 'subagent' || stoppable) && (
                <span className="relative grid size-6 shrink-0 place-items-center">
                  ...
                  {stoppable && (
                    ...
                    >
                      <ShidouIcon className="size-3 text-[var(--text-tertiary)]" name="stopFilled" />
                    </button>
                  )}
                </span>
              )}
...
function EnvironmentAction({
  ...
  return (
    <button
      ...
      onClick={onClick}
    >
      <ShidouIcon className="size-3.5 text-[var(--text-secondary)]" name={icon} />
      <span className="min-w-0 flex-1 truncate text-left">{label}</span>
      {trailing && <ShidouIcon className="size-[13px] text-[var(--text-tertiary)]" name={trailing} />}
    </button>
  )
}
  ...
  return (
    <div className="flex min-h-0 flex-1 items-center justify-center px-8 pb-12">
      <div className="max-w-sm text-center">
        <ShidouIcon className="mx-auto size-6 text-ring" name="sparkle" />
        ...
        <div className="mt-5 flex flex-col items-center gap-2">
          <Button className="rounded-full" onClick={onAddProject}>
            <ShidouIcon name="plus" /> {t('sidebar.add_project')}
          </Button>
          <Button
