// Swift port of the desktop's `src/md/mend.rs`, kept algorithm-for-algorithm
// with it: a marker that hangs mid-stream is closed for the *display* parse
// only, so styling settles the moment content follows an opener instead of
// flipping when the closer finally arrives. The canonical source is never
// touched, so a marker that genuinely never closes settles honestly — one
// flip at the end of the turn rather than jitter throughout.
//
// The scanner is deliberately approximate. Any mid-stream misjudgement is
// repaired by the very next delta or by the settle, and the cost of being
// exact is not worth paying on every chunk.

import Foundation

public enum MarkdownMend {
    /// Destination for a link whose URL is still streaming.
    public static let pendingLinkURL = "shidou:pending-link"
