...
When the commit dialog's message box is left empty, Shidou generates the subject
line by running **the session's own agent CLI** once, headlessly. No coding
agent exposes a commit-message endpoint and Shidou holds no provider API key, so
...
[`AgentInvocation`](../crates/shidou-protocol/src/git.rs#L45) alongside the probed
...
[`commit_prompt`](../crates/shidou-core/src/git_commit.rs#L173) builds it from Git
...
- **Amp** has no flag that disables tools, so Shidou writes a throwaway settings
  ...
  the error path ([git_commit.rs:90](../crates/shidou-core/src/git_commit.rs#L90)).
...
- **Claude Code** runs in `plan` permission mode with an empty `--tools` list,
  ...
  tiers in Shidou's catalog, but the CLI still accepts `--effort low` for it.
...
- **Grok** is the exception to prompt-last: the prompt is the value of
  ...
  ([git_commit.rs:303](../crates/shidou-core/src/git_commit.rs#L303)).
...
([git_commit.rs:691](../crates/shidou-core/src/git_commit.rs#L691)) walks
...
code fences, ANSI under `NO_COLOR`. [`normalize_message`](../crates/shidou-core/src/git_commit.rs#L349)
