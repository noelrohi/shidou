...
[`AgentSession`](../crates/shidou-protocol/src/model.rs) carries two:
...
2. **Polled from a native store.** The agent writes the title to disk (or to a
   ...
   [`NativeTitleRefresh`](../crates/shidou-core/src/driver/title_refresh.rs) on a
   backoff schedule, latching `RESOLVED` on the first hit. Claude, Amp, and
   Grok work this way.
3. **Generated by Shidou.** Only Codex.
...
| --- | --- | --- | --- | --- |
| Claude Code | Claude (Haiku 4.5) | transcript JSONL on disk | poll from each prompt, plus turn end | [title_refresh.rs:53](../crates/shidou-core/src/driver/title_refresh.rs#L53), [claude.rs:1300](../crates/shidou-core/src/driver/claude.rs#L1300) |
| Codex CLI | **Shidou** (`gpt-5.6-luna`, effort `low`) | second `codex app-server` process | first `turn/started`, new threads only | [codex.rs:636](../crates/shidou-core/src/driver/codex.rs#L636) |
| Codex CLI | Codex | JSON-RPC stream | thread start/resume, `thread/name/updated` | [codex.rs:1612](../crates/shidou-core/src/driver/codex.rs#L1612), [codex.rs:1735](../crates/shidou-core/src/driver/codex.rs#L1735) |
...
`generate_session_title` to Sonnet while the turn stayed on Opus. Shidou does not
...
[`claude_session::session_metadata`](../crates/shidou-core/src/claude_session.rs#L27)
...
Shidou writes when it forks a session — and it wins by being later in the file.

Shidou reads it on two paths:

- [`start_claude_title_refresh`](../crates/shidou-core/src/driver/claude.rs#L124),
  ...
  for a slow start.
- The turn-end read at [claude.rs:1295](../crates/shidou-core/src/driver/claude.rs#L1295),
...
### Codex — the one Shidou generates

The Codex app-server **persists** a thread name but never generates one; naming
is the client's job, and Codex Desktop does it client-side too. So Shidou matches
that behavior in
[`generate_codex_title`](../crates/shidou-core/src/driver/codex.rs#L1006): a
second, short-lived `codex app-server --stdio` process runs one ephemeral,
read-only turn.

- Model `gpt-5.6-luna` ([codex.rs:958](../crates/shidou-core/src/driver/codex.rs#L958)),
...
[`amp_session::thread_title`](../crates/shidou-core/src/amp_session.rs#L49) shells
...
`generated_title` field ([grok_session.rs:22](../crates/shidou-core/src/grok_session.rs#L22)).
...
([acp.rs:1305](../crates/shidou-core/src/driver/acp.rs#L1305)): it emits one at
the start of the first turn and Shidou forwards the `title` as `AutoTitleUpdated`,
no polling and no extra process.

What it sends, though, is **the first prompt verbatim** — the same content as
Shidou's own fallback, arriving through a different door. Kimi files it in its
