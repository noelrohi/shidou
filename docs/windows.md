...
`https://releases.shidou.dev/latest-windows.txt` names the current version if you
...
Nothing else: Shidou links the C runtime statically, so there is no Visual C++
...
Shidou updates itself. It checks once per launch, and an available update
appears in the sidebar footer; clicking it downloads the installer, verifies
its signature, and runs it. Shidou closes, is replaced in place, and reopens.
...
- `https://releases.shidou.dev/appcast-windows-x86_64.xml`
- `https://releases.shidou.dev/appcast-windows-aarch64.xml`

Every installer carries an EdDSA signature, and Shidou refuses one that does not
...
`%LOCALAPPDATA%\Shidou\updater.json`.

## Where Shidou keeps its data
...
Shidou detects the provider CLIs on `PATH` and, because a fresh `PATH` may
...
Shidou hosts it in *visual* mode rather than as a child window: the page renders
...
- **Pen, touch and dragging files into the page are not wired up.** Visual
  hosting delivers no input of its own; Shidou forwards mouse, wheel, cursor and
...
**The window opens black, or the app exits at startup.** Shidou needs a working
...
shim on `PATH`. If the shell finds it but Shidou does not, set the binary path in
**Settings → Providers** and file an issue with the install method.

**Git-backed features do nothing.** Shidou shells out to `git`. Install Git for
Windows and make sure `git --version` works in a new terminal.

**The update never arrives.** Shidou reaches the feed with the `curl.exe` in
System32; a proxy or filter that blocks `releases.shidou.dev` blocks updates too.
