...
impl ProviderKind {
    ...
    /// fork or truncation method. Neither can reproduce Shidou's "drop the last N
...
///
/// Drafts may carry [`Self::NewWorktree`] until their first prompt. Shidou then
...
pub struct QueuedMessage {
    pub id: Uuid,
    pub content: String,
    /// The text typed before Shidou appended provider-facing attachment
...
pub enum DriverEvent {
    ...
    /// fresh Harness session may resolve its deployment default when Shidou did
...
mod tests {
    ...
    fn attachment_messages_keep_transport_and_visible_content_separate() {
        let project = Project::from_path(PathBuf::from("/tmp/shidou"));
        ...
        let attachment = MessageAttachment {
            ...
            is_dir: false,
            is_image: true,
            blob_reference: Some("shidou-blob:ab/reference.png".to_owned()),
        };
    ...
    fn activity_targets_are_normalized_when_events_arrive() {
        let cases = [
            (
                ActivityKind::FileRead,
                serde_json::json!({"input": {"file_path": "/tmp/shidou/src/app.rs"}}),
                "/tmp/shidou/src/app.rs",
            ),
            (
                ActivityKind::FileSearch,
            ...
            (
                ActivityKind::FileList,
                serde_json::json!({"arguments": {"directory": "/tmp/shidou/src"}}),
                "/tmp/shidou/src",
            ),
            (
            ...
            (
                ActivityKind::Search,
                serde_json::json!({"action": {"queries": ["Shidou GPUI"]}}),
                "Shidou GPUI",
            ),
            (
                ActivityKind::FileRead,
                serde_json::json!("/tmp/shidou/README.md"),
                "/tmp/shidou/README.md",
            ),
        ];
    ...
    fn projectless_projects_use_projects_root_and_recognize_legacy_paths() {
        ...
        let ordinary = Project::from_path(home.join("dev/shidou"));
    ...
    fn legacy_file_edit_details_are_promoted_to_arguments_and_metadata() {
        ...
        assert_eq!(activities[0].file_changes[0].path, "/tmp/shidou/README.md");
    ...
    fn legacy_file_tools_are_reclassified_and_gain_cached_targets() {
        ...
        cached.display_target = Some("/tmp/shidou/src/persisted.rs".into());
        session.transcript_blocks.push(TranscriptBlock {
            after_message: 0,
            turn_id: None,
            activities: vec![
                ActivityItem::new(
                    ...
                    ActivityKind::Search,
                    "read",
                    Some(r#"{"filePath":"/tmp/shidou/src/model.rs"}"#.into()),
                    true,
                ),
                ActivityItem::new(
                    None,
        ...
        assert_eq!(
            ...
            Some("/tmp/shidou/src/persisted.rs")
        );
        assert!(
            activities[..2]
                .iter()
