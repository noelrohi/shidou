...
use shidou_protocol::{Command, ResponsePayload, WorkspaceOperation, WorkspaceResult};
use uuid::Uuid;
...
impl WorkspaceClient {
    ...
    pub fn request(&self, operation: WorkspaceOperation) -> anyhow::Result<WorkspaceResult> {
            ...
            .request(Uuid::nil(), Uuid::nil(), Command::Workspace { operation })?
        {
            ...
            _ => anyhow::bail!("Shidou daemon returned an invalid workspace response"),
        }
    }
    ...
    pub fn fork_provider_session(
        ...
        match self.client.request(
            ...
            _ => anyhow::bail!("Shidou daemon returned an invalid provider-fork response"),
        }
    }
}
