...
fn main() -> anyhow::Result<()> {
    ...
    let token =
        std::env::var(DAEMON_TOKEN_ENV).context("Shidou daemon authentication token is missing")?;
    ...
    let listener = TcpListener::bind(&arguments.bind)
        .with_context(|| format!("could not bind Shidou daemon to {}", arguments.bind))?;
    ...
    if let Some(parent_pid) = arguments.parent_pid {
        ...
        std::thread::Builder::new()
            .name("shidou-daemon-parent".into())
            .spawn(move || {
    ...
    let task_path = shidou_core::persistence::StateStore::default_path();
    let settings = shidou_core::DaemonSettingsStore::open_with_legacy(
        shidou_core::DaemonSettings::default_path(),
        [task_path.with_file_name("settings.json")],
    )
    .context("could not load daemon settings")?;
    let task_store = shidou_core::persistence::StateStore::daemon(task_path);
    shidou_core::serve(
        listener,
        token,
        Arc::new(shidou_core::daemon::ShidouBackend::new(
            settings, task_store,
        )?),
        shutdown,
...
mod tests {
    ...
    fn parses_repeated_browser_origin_allowlist_entries() {
        ...
        assert_eq!(
            ...
            ["https://app.shidou.test", "http://localhost:3000"]
        );
