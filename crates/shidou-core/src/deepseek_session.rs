...
// Rust destructors. Keep one pipe open in Shidou and let this wrapper terminate
...
impl DeepSeekServer {
    ...
    fn start_with_dsh_home(binary: &Path, dsh_home: Option<&Path>) -> anyhow::Result<Self> {
        ...
        let mut command = {
            ...
            command
                .args(["-c", DSH_GUARDIAN_SCRIPT, "shidou-dsh-guardian"])
                .arg(binary)
        ...
        thread::Builder::new()
            .name("shidou-deepseek-stdout".into())
            .spawn(move || {
        ...
        thread::Builder::new()
            .name("shidou-deepseek-stderr".into())
            .spawn(move || {
        ...
        for (name, path) in [("mux", "/api/events.mux"), ("host", "/api/events.host")] {
            ...
            thread::Builder::new()
                .name(format!("shidou-deepseek-{name}-events"))
        ...
        thread::Builder::new()
            .name("shidou-deepseek-process-monitor".into())
            .spawn(move || {
                loop {
                    ...
                    match status {
                        Ok(Some(status)) => {
                            monitor_events.publish(json!({
                                "type": "server-request",
                                "rpcId": format!("shidou-process-{}", Uuid::new_v4()),
                                "method": "shidou/process-exited",
                                "payload": {
                                    "type": "shidou/process-exited",
    ...
    pub(crate) fn rpc(&self, method: &str, payload: Value) -> anyhow::Result<Value> {
        let rpc_id = format!("shidou-{}", Uuid::new_v4());
        let body = json!({
            "type": "client-request",
            "rpcId": rpc_id,
            "method": method,
...
fn stream_error(message: String) -> Value {
    json!({
        "type": "server-request",
        "rpcId": format!("shidou-stream-{}", Uuid::new_v4()),
        "method": "stream/error",
        "payload": {
            "type": "stream/error",
...
mod tests {
    ...
    /// Exercises Shidou's HTTP envelope, WebSocket handshakes, and process-tree
    ...
    fn installed_harness_host_answers_rpc() {
        ...
        #[cfg(unix)]
        {
            // Simulate an abruptly terminated Shidou process: the OS closes its
    ...
    fn installed_harness_session_contracts_round_trip() {
        ...
        std::fs::create_dir_all(&root.0).unwrap();
        {
            ...
            let created = server
                ...
                .expect("session.create should accept Shidou's payload");
            assert_eq!(
            ...
            let history = server
                ...
                .expect("session.history should accept Shidou's payload");
