...
#[cfg(unix)]
const SHELL_ENV_COMMAND: &str = "/usr/bin/env -0 > \"$SHIDOU_SHELL_ENV_CAPTURE_FILE\"";
...
/// Build a command with the environment a terminal-launched Shidou normally
...
/// The `PATH` a provider CLI runs with: every directory Shidou itself searched,
...
/// GUI-launched Shidou can still be running with only the `PATH` it inherited.
...
///
/// Shidou's Windows build is a GUI-subsystem binary with no console of its own,
...
/// Normalize a Shidou-owned provider thread before a dependency spawns the child
...
/// Shidou inherits explorer's `PATH`, which predates later installs, and the
...
foreach ($target in @('User', 'Machine')) {
  ...
  if ($value) { $entries.Add('SHIDOU_' + $target.ToUpper() + '_PATH=' + [Environment]::ExpandEnvironmentVariables($value)) }
}
[IO.File]::WriteAllText($env:SHIDOU_SHELL_ENV_CAPTURE_FILE, [string]::Join([string][char]0, $entries))
";
...
fn merge_windows_environment(mut environment: ShellEnvironment) -> Option<ShellEnvironment> {
    ...
    for name in ["PATH", "SHIDOU_USER_PATH", "SHIDOU_MACHINE_PATH"] {
...
pub fn default_terminal_shell() -> PathBuf {
    ...
    // see the note in shidou-client's `unix_terminal_shell_candidates`. The
...
impl ShellEnvironmentCapture {
    fn create() -> Option<Self> {
        for _ in 0..16 {
            ...
            let path =
                std::env::temp_dir().join(format!(".shidou-shell-env-{}-{id}", std::process::id()));
...
mod tests {
    ...
    fn a_provider_cli_runs_with_the_directories_detection_searched() {
        #[cfg(windows)]
        let program = PathBuf::from("C:\\shidou-fixture\\bin\\pi.exe");
        #[cfg(not(windows))]
        let program = PathBuf::from("/opt/shidou-fixture/bin/pi");
    ...
    fn spawn_unblocks_sigchld_in_the_child_and_restores_the_caller() {
        if std::env::var_os("SHIDOU_SIGCHLD_CHILD_PROBE").is_some() {
        ...
        command
            ...
            .env("SHIDOU_SIGCHLD_CHILD_PROBE", "1");
    ...
    fn windows_profile_environment_merges_registry_paths_behind_the_profile_path() {
        let environment = merge_windows_environment(vec![
            ...
            (
                OsString::from("SHIDOU_USER_PATH"),
                OsString::from("C:\\user;C:\\shared"),
            ),
            (
                OsString::from("SHIDOU_MACHINE_PATH"),
        ...
        assert!(
            !environment
                .iter()
                .any(|(name, _)| name == OsStr::new("SHIDOU_USER_PATH"))
        );
        assert!(
            !environment
                .iter()
                .any(|(name, _)| name == OsStr::new("SHIDOU_MACHINE_PATH"))
        );
    }
    ...
    fn a_bare_name_resolves_through_pathext() {
        let directory = std::env::temp_dir().join(format!("shidou-pathext-{}", std::process::id()));
    ...
    fn a_bare_name_prefers_pathext_over_an_extensionless_shim() {
        ...
        let directory =
            std::env::temp_dir().join(format!("shidou-pathext-shim-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&directory);
        std::fs::create_dir_all(&directory).expect("create fixture directory");
    ...
    fn parses_null_delimited_environment_without_losing_value_contents() {
        let environment = parse_shell_environment(
            b"PATH=/Users/example/.fnm/current/bin:/usr/bin\0TOKEN=line one\nline two=rest\0EMPTY=\0SHIDOU_SHELL_ENV_CAPTURE_FILE=/tmp/capture\0",
        )
    ...
    fn captures_environment_from_a_shell_process() {
        ...
        fs::write(
            &shell,
            "#!/bin/sh\n/usr/bin/printf 'PATH=/Users/example/.fnm/current/bin:/usr/bin\\000SHIDOU_TEST_TOKEN=from-shell\\000' > \"$SHIDOU_SHELL_ENV_CAPTURE_FILE\"\n",
        )
        ...
        assert_eq!(
            ...
            vec![
                ...
                (
                    OsString::from("SHIDOU_TEST_TOKEN"),
