...
const TURN_START_METADATA_PREFIX: &str = "Shidou-Turn-Start: ";
...
pub fn checkpoint_ref(session_id: Uuid, turn_count: usize) -> String {
    format!("refs/shidou/session-{session_id}-turn-{turn_count}")
}

pub fn turn_start_ref(session_id: Uuid, turn_count: usize) -> String {
    format!("refs/shidou/session-{session_id}-turn-start-{turn_count}")
}

pub fn turn_diff_base_ref(session_id: Uuid, turn_count: usize) -> String {
    format!("refs/shidou/session-{session_id}-turn-diff-{turn_count}")
}
...
pub fn capture_turn(cwd: &Path, session_id: Uuid, turn_count: usize) -> anyhow::Result<Checkpoint> {
    ...
    let end_commit =
        capture_worktree_commit_from(cwd, end_head.as_deref(), "Shidou worktree snapshot", &[])?;
...
pub fn capture_worktree_commit(cwd: &Path) -> anyhow::Result<String> {
    ...
    let head = resolve_ref(cwd, "HEAD");
    capture_worktree_commit_from(cwd, head.as_deref(), "Shidou worktree snapshot", &[])
}
...
fn capture_worktree_commit_from(
    ...
    let temporary_index = common_dir.join(format!("shidou-checkpoint-index-{}", Uuid::new_v4()));
...
fn virtual_branch_start(
    ...
    commit_tree(cwd, tree, "Shidou turn diff base", &[])
}

fn empty_tree_commit(cwd: &Path) -> anyhow::Result<String> {
    commit_tree(cwd, EMPTY_TREE, "Shidou empty turn diff base", &[])
}
...
fn session_checkpoint_ref_commits(cwd: &Path, session_id: Uuid) -> SessionCheckpointRefs {
    let prefix = format!("refs/shidou/session-{session_id}-");
    git_output(
        cwd,
        [
            "for-each-ref",
    ...
    S: AsRef<OsStr>,
{
    ...
    if identity {
        command
            .env("GIT_AUTHOR_NAME", "Shidou")
            .env("GIT_AUTHOR_EMAIL", "shidou@localhost")
            .env("GIT_COMMITTER_NAME", "Shidou")
            .env("GIT_COMMITTER_EMAIL", "shidou@localhost");
    }
...
mod tests {
    ...
    fn diverged_repository() -> PathBuf {
        let directory = std::env::temp_dir().join(format!("shidou-checkpoints-{}", Uuid::new_v4()));
        fs::create_dir_all(&directory).unwrap();
        git_ok(&directory, &["init", "--quiet", "--initial-branch=main"]);
        git_ok(&directory, &["config", "user.name", "Shidou Test"]);
        git_ok(&directory, &["config", "user.email", "shidou@example.com"]);
    ...
    fn session_turn_refs_lists_the_sessions_checkpoints_in_one_call() {
        ...
        git_ok(
            ...
            &[
                "-c",
                "user.name=Shidou Test",
                "-c",
                "user.email=shidou@example.com",
                "commit",
                "--quiet",
                "-m",
                "baseline",
