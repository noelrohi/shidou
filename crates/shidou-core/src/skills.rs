...
pub use shidou_protocol::skills::{
...
mod tests {
    ...
    fn temp_root(tag: &str) -> PathBuf {
        let root = std::env::temp_dir().join(format!("shidou-skills-{tag}-{}", std::process::id()));
    ...
    fn scan_orders_user_before_projects_and_counts_cross_scope_duplicates() {
        ...
        assert_eq!(
            names,
            vec![("review", None), ("zeta", None), ("review", Some("shidou"))]
        );
    ...
    fn every_ecosystem_root_is_listed() {
        let projects = vec![("shidou".to_owned(), PathBuf::from("/tmp/shidou"))];
        ...
        for expected in [
            "/tmp/shidou/.agents/skills",
            "/tmp/shidou/.claude/skills",
            "/tmp/shidou/.codex/skills",
            "/tmp/shidou/.opencode/skills",
            "/tmp/shidou/.cursor/skills",
            "/tmp/shidou/.pi/skills",
        ] {
            assert!(
