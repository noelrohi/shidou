...
fn build_opencode_computer_use_config(
    ...
    mcp.insert(
        ...
        serde_json::json!({
            ...
            "enabled": true,
            "environment": {
                "SHIDOU_COMPUTER_USE_SERVER": server_path.display().to_string(),
                "SHIDOU_COMPUTER_USE_PROCESS_DIRECTORY": process_directory.display().to_string(),
            },
        }),
    );
...
pub(super) fn opencode_computer_use_environment(
    ...
    vec![
        ...
        (
            "SHIDOU_COMPUTER_USE_SERVER".to_owned(),
            base.server_path.display().to_string(),
        ),
        (
            "SHIDOU_COMPUTER_USE_PROCESS_DIRECTORY".to_owned(),
...
fn build_grok_computer_use_config(
    ...
    let rules = fs::read_to_string(&base.skill_path).with_context(|| {
        format!(
            "could not read Shidou Computer Use skill {}",
...
fn build_grok_computer_use_toml(
    ...
    environment.insert(
        "SHIDOU_COMPUTER_USE_SERVER".into(),
        toml::Value::String(base.server_path.display().to_string()),
    );
    environment.insert(
        "SHIDOU_COMPUTER_USE_PROCESS_DIRECTORY".into(),
    ...
    mcp_servers.insert("shidou_js_repl".into(), toml::Value::Table(server));
...
mod tests {
    ...
    fn computer_use_config() -> computer_use_runtime::ComputerUseConfig {
        computer_use_runtime::ComputerUseConfig {
            ...
            repl_path: PathBuf::from("/Applications/Shidou.app/Contents/Resources/shidou_js_repl"),
            skill_path: PathBuf::from(
                "/Applications/Shidou.app/Contents/Resources/skills/shidou-computer-use/SKILL.md",
            ),
    ...
    fn opencode_computer_use_config_preserves_existing_inline_config() {
        let content = build_opencode_computer_use_config(
            ...
            Path::new("/Applications/Shidou Computer Use"),
            Path::new("/Applications/Shidou.app/Contents/Resources/shidou_js_repl"),
            Path::new(
                "/Applications/Shidou.app/Contents/Resources/skills/shidou-computer-use/SKILL.md",
            ),
            Path::new("/tmp/shidou computer use/session"),
        )
        .unwrap();
        ...
        assert_eq!(
            value
                .pointer("/mcp/shidou_js_repl/command/0")
                .and_then(Value::as_str),
            Some("/Applications/Shidou.app/Contents/Resources/shidou_js_repl")
        );
        assert_eq!(
            value
                .pointer("/mcp/shidou_js_repl/environment/SHIDOU_COMPUTER_USE_SERVER")
                .and_then(Value::as_str),
            Some("/Applications/Shidou Computer Use")
        );
        assert_eq!(
        ...
        assert!(value.pointer("/mcp/shidou_computer_use").is_none());
    }

    #[test]
    fn grok_computer_use_config_preserves_existing_config_and_replaces_shidou_server() {
        ...
        let server = value
            .get("mcp_servers")
            .and_then(|mcp| mcp.get("shidou_js_repl"))
            .unwrap();
        assert_eq!(
        ...
        assert_eq!(
            server
                .get("env")
                .and_then(|env| env.get("SHIDOU_COMPUTER_USE_SERVER"))
                .and_then(toml::Value::as_str),
            Some("/tmp/Shidou Computer Use")
        );
    }
