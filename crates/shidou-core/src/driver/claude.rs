...
fn stop_task_request(request_id: u64, task_id: &str) -> Value {
    json!({
        ...
        "request_id": format!("shidou-{request_id}"),
...
impl ClaudeDriver {
    pub fn start(options: DriverStartOptions, events: DriverEventSender) -> anyhow::Result<Self> {
        ...
        thread::Builder::new()
            .name("shidou-claude-writer".into())
            .spawn(move || {
                let mut stdin = stdin;
                    ...
                    let written = match message {
                        ...
                        CommandMessage::Cancel => {
                            ...
                            write_line(
                                ...
                                &json!({
                                    "type": "control_request",
                                    "request_id": format!("shidou-{next_request_id}"),
                        ...
                        CommandMessage::StopBackgroundWork { key, control_id } => {
                            next_request_id += 1;
                            let request_id = format!("shidou-{next_request_id}");
        ...
        let stderr_thread = thread::Builder::new()
            .name("shidou-claude-stderr".into())
            .spawn(move || {
        ...
        thread::Builder::new()
            .name("shidou-claude-process".into())
            .spawn(move || {
...
fn start_claude_task_output_tail(
    ...
    let spawned = thread::Builder::new()
        .name("shidou-claude-task-output".into())
        .spawn(move || {
...
fn handle_message(
    ...
    match message_type {
        ...
        Some("user") => {
            ...
            // `--replay-user-messages` echoes Shidou's own prompts back; they are
...
mod tests {
    ...
    fn locates_claudes_native_task_output_across_workspace_slugs() {
        let root =
            std::env::temp_dir().join(format!("shidou-claude-output-test-{}", Uuid::new_v4()));
        let output = root
            .join("-Users-egoist-dev-shidou")
            .join("session-live")
            .join("tasks")
    ...
    fn native_task_output_streams_before_completion() {
        let root = std::env::temp_dir().join(format!("shidou-claude-tail-test-{}", Uuid::new_v4()));
    ...
    fn stop_task_control_errors_restore_the_live_item() {
        ...
        state
            .pending_task_stops
            .lock()
            .insert("shidou-8".into(), key.clone());
        handle_message(
            &json!({
                "type": "control_response",
                "response": {
    ...
    fn background_bash_task_keeps_its_originating_command() {
        ...
        handle_message(
            &json!({
                ...
                "message": {"content": [{
                    ...
                    "name": "Bash",
                    "input": {
                        "command": "cargo check && cargo test --bin shidou",
        ...
        assert_eq!(
            ...
            Some("cargo check && cargo test --bin shidou")
        );
    }
    ...
    fn streams_text_and_tools_and_ignores_its_own_replayed_prompt() {
        ...
        let wire = [
            ...
            // Shidou's own prompt, echoed by --replay-user-messages.
