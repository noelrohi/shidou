...
//! permission requests the user can actually be asked. Shidou already started
...
impl OpenCodeDriver {
    pub fn start(options: DriverStartOptions, events: DriverEventSender) -> anyhow::Result<Self> {
        ...
        // Shidou must therefore install the selected access policy on this
        ...
        thread::Builder::new()
            .name("shidou-opencode-usage-metadata".into())
            .spawn(move || {
        ...
        thread::Builder::new()
            .name("shidou-opencode-events".into())
            .spawn(move || {
                ...
                match open_event_stream(stream_port, "/event", &stream_control) {
                    Ok(Some(stream)) => {
                        ...
                        // lost when Shidou reconnects after it was asked. Events
        ...
        thread::Builder::new()
            .name("shidou-opencode-driver".into())
            .spawn(move || {
...
fn rehydrate_pending_permissions(
    ...
    // `session.idle` settles Shidou's persisted running turn exactly once.
...
fn request_permission(
    ...
    // so Shidou sends only one-shot provider replies and retains durable choices
...
mod tests {
    ...
    /// `cargo test --bin shidou opencode_session_against_a_real_server -- --ignored`.
    #[test]
    ...
    fn permission_approvals_stay_driver_local() {
        ...
        let repeated = json!({
            ...
            "properties": {
                ...
                "sessionID": "ses_1",
                "permission": "bash",
                "patterns": ["rm -rf /tmp/shidou-cache"],
                "metadata": {},
                "always": ["rm -rf *"]
            }
        });
