...
//! unknown-method errors, stdio lifetime, and protocol type validation. Shidou
...
fn launch_for(provider: ProviderKind) -> anyhow::Result<AcpLaunch> {
    match provider {
        ...
        ProviderKind::Grok => Ok(AcpLaunch {
            ...
            env: vec![("GROK_OAUTH2_REFERRER".into(), "shidou".into())],
        }),
...
impl AcpDriver {
    pub fn start(
        ...
        thread::Builder::new()
            .name(format!("shidou-{}-acp", provider.id()))
            .spawn(move || {
    ...
    Client
        ...
        .connect_with(agent, async move |connection: ConnectionTo<Agent>| {
            ...
            if provider == ProviderKind::Cursor {
                ...
                // clients that opt in. Shidou applies the returned config option
            ...
            let initialize = connection
                .send_request(
                    InitializeRequest::new(ProtocolVersion::V1)
                        ...
                        .client_info(Implementation::new("shidou", env!("CARGO_PKG_VERSION"))),
                )
                .block_task()
                .await?;
...
/// agents Shidou drives keep it on `mode`.
...
fn send_prompt(
    ...
    let extension_id =
        (provider == ProviderKind::Grok).then(|| format!("shidou-{}", uuid::Uuid::new_v4()));
...
fn handle_session_update(
    ...
    match kind {
        ...
        // `user_message_chunk` is Shidou's own prompt echoed back. Other typed
...
mod tests {
    ...
    fn typed_updates_preserve_text_reasoning_and_correlated_tools() {
        ...
        let updates = [
            ...
            json!({"sessionUpdate":"tool_call_update","toolCallId":"call_1","status":"completed","title":"fixture.txt","content":[{"type":"content","content":{"type":"text","text":"shidou probe fixture"}}]}),
        ...
        assert!(matches!(&seen[2], DriverEvent::RichActivity(item)
                if item.complete
                    ...
                    && item.output.as_deref().is_some_and(|output| output.contains("shidou probe fixture"))));
