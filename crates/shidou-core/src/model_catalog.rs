...
fn discover_pi_models(binary: &Path, dialect: PiDialect) -> Vec<ProviderModel> {
    ...
    let models_request = json!({"id": "shidou-models", "type": "get_available_models"});
    let result = if write_json_line(&mut stdin, &models_request).is_ok()
        && let Some(models) = recv_pi_rpc_response(&rx, "shidou-models", PI_RPC_TIMEOUT)
    {
        ...
        let state_request = json!({"id": "shidou-state", "type": "get_state"});
        let state = if write_json_line(&mut stdin, &state_request).is_ok() {
            recv_pi_rpc_response(&rx, "shidou-state", PI_RPC_TIMEOUT).unwrap_or(Value::Null)
        } else {
            Value::Null
        };
...
mod tests {
    ...
    fn write_fake_pi(name: &str, contents: &str) -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "shidou-{name}-{}-pi-model-discovery.sh",
            std::process::id()
        ));
    ...
    fn model_cache_round_trips_and_rejects_empty_or_invalid_files() {
        let directory =
            std::env::temp_dir().join(format!("shidou-model-cache-test-{}", std::process::id()));
...
while IFS= read -r request; do
  case "$request" in
    *shidou-models*)
      printf '%s\n' '{"id":"shidou-models","type":"response","success":true,"data":{"models":[{"provider":"extension-provider","id":"extension-model","name":"Extension Model","reasoning":false}]}}'
      ;;
    *shidou-state*)
      printf '%s\n' '{"id":"shidou-state","type":"response","success":true,"data":{"model":{"provider":"extension-provider","id":"extension-model"},"thinkingLevel":"medium"}}'
      ;;
  esac
done
"#,
