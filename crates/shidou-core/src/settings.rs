...
pub use shidou_protocol::settings::DaemonSettings;
use uuid::Uuid;
        ...
        let (mut settings, write_current) = match fs::read(&path) {
            Ok(bytes) => match serde_json::from_slice(&bytes) {
                ...
                Err(error) => {
                    ...
                    eprintln!(
                        "Shidou daemon moved invalid settings to {}: {error}",
                        backup.display()
                    );
...
mod tests {
    ...
    use shidou_protocol::model::ProviderKind;
    use uuid::Uuid;
    ...
    fn imports_daemon_fields_from_a_debug_combined_settings_file() {
        let directory = std::env::temp_dir().join(format!("shidou-settings-{}", Uuid::new_v4()));
