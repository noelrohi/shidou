//! Provider backend and driver-event wire translation for `shidou-daemon`.
...
fn migrate_projectless_state(
    ...
    for index in indices {
        ...
        let workspace = crate::projectless::migrate_workspace(&old_path).with_context(|| {
            format!(
                "could not move projectless workspace {} under ~/.shidou/projects",
                old_path.display()
            )
        })?;
...
impl Backend for ShidouBackend {
        ...
        match request.command {
            ...
            Command::Start { options } => {
                ...
                std::thread::Builder::new()
                    .name(format!("shidou-daemon-events-{session_id}"))
                    .spawn(move || {
...
mod tests {
    ...
    fn client_projection_cannot_replace_a_daemon_checkpoint() {
        ...
        let checkpoint = Checkpoint {
            turn_count: 1,
            git_ref: "refs/shidou/canonical".into(),
        ...
        incoming.turns[0].checkpoint = Some(Checkpoint {
            git_ref: "refs/shidou/stale-client".into(),
            ..checkpoint.clone()
        });
        preserve_daemon_checkpoints(&existing, &mut incoming);
