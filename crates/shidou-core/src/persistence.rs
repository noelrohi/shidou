...
//! `~/.shidou/app.json` for Release, daemon preferences in
//! `~/.shidou/settings.json`, and binary payloads in [`crate::blob_store`].
//! Of the configuration documents, only the desktop file is written here;
//! daemon settings cross the RPC boundary and are persisted by `shidou-daemon`.
//!
...
/// at `~/.shidou/app.json` without exposing app-managed state or daemon-owned
/// provider policy.
...
pub struct StateStore {
    ...
    /// Release uses the explicit cross-client Shidou configuration directory.
...
impl StateStore {
    ...
    /// Local database owner used inside `shidou-daemon`. It never reads or
...
mod tests {
    ...
    fn temporary_directory() -> PathBuf {
        std::env::temp_dir().join(format!("shidou-state-{}", Uuid::new_v4()))
    }
    ...
    fn load_returns_list_columns_and_hydrate_fills_the_transcript() {
        ...
        state.sessions[0].workspace = SessionWorkspace::Worktree {
            ...
            branch: "shidou/investigate".into(),
        };
    ...
    fn sent_attachment_presentation_round_trips_with_message_rows() {
        ...
        let attachment = MessageAttachment {
            ...
            is_dir: false,
            is_image: true,
            blob_reference: Some("shidou-blob:abcdef.png".to_owned()),
        };
    ...
    fn default_path_is_build_specific() {
        ...
        {
            assert_eq!(directory, Some(std::ffi::OsStr::new("Shidou")));
            let configuration_directory = dirs::home_dir()
                .unwrap_or_else(std::env::temp_dir)
                .join(".shidou");
            assert_eq!(
