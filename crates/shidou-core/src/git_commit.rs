...
pub use shidou_protocol::git::{AgentInvocation, CommitSnapshot as Snapshot};
...
mod tests {
    ...
    fn repository() -> PathBuf {
        let root = std::env::temp_dir().join(format!("shidou-commit-test-{}", Uuid::new_v4()));
        fs::create_dir_all(&root).unwrap();
        run_git(&root, &["init", "-b", "main"]);
        run_git(&root, &["config", "user.name", "Shidou Tests"]);
        run_git(&root, &["config", "user.email", "shidou@example.com"]);
