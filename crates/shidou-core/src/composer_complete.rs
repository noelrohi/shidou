...
/// Shidou's own built-ins. The shared commands are plain prompt templates Shidou
...
fn builtin_shidou_commands(provider: ProviderKind) -> Vec<SlashCommand> {
...
/// - Codex: `~/.codex/prompts`, expanded by Shidou at submit.
...
/// `/skill:name` there.
///
/// On top of the native sources, every provider reads Shidou's own layer —
/// `.shidou/commands` in the project and `~/.config/shidou/commands` — and gets
/// Shidou's built-ins ([`builtin_shidou_commands`]), always expanded by Shidou, so
...
pub fn discover_slash_commands(provider: ProviderKind, project_root: &Path) -> Vec<SlashCommand> {
    ...
    match provider {
        ...
        // rather than from files Shidou could scan.
    ...
    // Shidou lists it for every provider.
    scan_skill_files(
        provider,
    ...
    scan_command_files(
        &project_root.join(".shidou/commands"),
        CommandScope::Project,
        true,
        &mut commands,
    );
    if let Some(home) = home.as_deref() {
        scan_command_files(
            &home.join(".config/shidou/commands"),
            CommandScope::User,
            true,
            &mut commands,
        );
    }
    commands.extend(builtin_shidou_commands(provider));
...
mod tests {
    ...
    fn every_provider_offers_slash_commands_out_of_the_box() {
        let root = std::env::temp_dir().join(format!("shidou-empty-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).unwrap();
        for provider in ProviderKind::ALL {
            ...
            for expected in ["init", "review", "commit"] {
                ...
                // everywhere else Shidou expands its template.
                if provider != ProviderKind::Claude {
                    assert!(
                        command.template.is_some(),
                        "{} /{expected} must be a Shidou template",
    ...
    fn fast_builtin_is_codex_only() {
        for provider in ProviderKind::ALL {
            let commands = builtin_shidou_commands(provider);
    ...
    fn shared_skills_are_listed_raw_on_every_provider() {
        let root = std::env::temp_dir().join(format!("shidou-skills-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn symlinked_skills_and_command_dirs_are_discovered() {
        let root = std::env::temp_dir().join(format!("shidou-symlink-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn codex_plugin_skill_uses_qualified_catalog_key() {
        let root =
            std::env::temp_dir().join(format!("shidou-codex-plugin-skill-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn derived_directories_join_the_file_index() {
        let root = std::env::temp_dir().join(format!("shidou-files-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
