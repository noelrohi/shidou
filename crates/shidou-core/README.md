...
`shidou-core` is Shidou's daemon-only runtime. It contains the native session
...
[`shidou-protocol`](../shidou-protocol), but contains no desktop transport or UI.
...
`DaemonClient` lives in [`shidou-client`](../shidou-client), which is what Shidou
Desktop depends on. `serve` and `ShidouBackend` are used by the `shidou-daemon`
binary.

Configuration ownership is explicit:

- the desktop owns `~/.shidou/app.json` in Release and checkout-local
  `temp/app.json` in Debug;
- the daemon owns `~/.shidou/settings.json`.
...
`shidou-protocol`, including checked-in TypeScript bindings.
