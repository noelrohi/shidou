...
impl Shidou {
                ...
                div().px(px(12.0)).child(
                    div()
                        ...
                        .on_click(cx.listener(|this, _, window, cx| {
                            ...
                            let focus_handle = this.main_composer_focus(cx);
    ...
    fn daemon_exposure_from_fields(
        &self,
        cx: &App,
    ) -> Result<shidou_client::DaemonExposureSettings, String> {
        let port = self
            .daemon_port_input
            .read(cx)
            .content()
        ...
        settings
            ...
            .and_then(shidou_client::DaemonExposureSettings::validate)
    ...
    fn regenerate_daemon_token(&mut self, cx: &mut Context<Self>) {
        ...
        settings.token = shidou_client::DaemonExposureSettings::new_token();
        self.daemon_token_revealed = false;
        self.apply_daemon_exposure(settings, cx);
    }
    ...
    fn apply_daemon_exposure(
        &mut self,
        settings: shidou_client::DaemonExposureSettings,
        cx: &mut Context<Self>,
    ) {
    ...
    fn set_ui_font_size(&mut self, size: f32, window: &mut Window, cx: &mut Context<Self>) {
        let size = shidou_client::persistence::sanitized_ui_font_size(size);
    ...
    fn set_code_font_size(&mut self, size: f32, cx: &mut Context<Self>) {
        let size = shidou_client::persistence::sanitized_code_font_size(size);
    ...
    pub(super) fn request_computer_permissions(&mut self, prompt: bool, cx: &mut Context<Self>) {
        ...
        std::thread::Builder::new()
            .name("shidou-computer-permission-request".into())
            .spawn(move || {
                let result = match daemon.request(
                    Uuid::nil(),
                    Uuid::nil(),
                    shidou_client::Command::ProbeComputerPermissions { prompt },
                ) {
