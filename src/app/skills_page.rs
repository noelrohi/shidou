...
//! ([`Shidou::ensure_skills_catalog`]); frames read only the cached catalog.
...
/// in [`Shidou::sync_skills_rows`]: a changed row — catalog identity, enabled
            ...
            let catalog = cx
                ...
                .spawn(async move {
                    match daemon.request(
                        Uuid::nil(),
                        Uuid::nil(),
                        shidou_client::Command::LoadSkills { projects },
                    )? {
                        shidou_client::ResponsePayload::SkillsCatalog { catalog } => Ok(catalog),
            ...
            let result = cx
                ...
                .spawn(async move {
                    daemon.request(
                        Uuid::nil(),
                        Uuid::nil(),
                        shidou_client::Command::SetSkillsEnabled { dirs, enabled },
                    )
                })
                .await;
            let _ = this.update(cx, |this, cx| {
            ...
            let result = cx
                ...
                .spawn(async move {
                    daemon.request(
                        Uuid::nil(),
                        Uuid::nil(),
                        shidou_client::Command::TrashSkills { dirs },
                    )
                })
                .await;
            let _ = this.update(cx, |this, cx| {
