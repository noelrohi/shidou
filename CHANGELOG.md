...
All notable changes to Shidou. This file is the **source of truth for the release
...
- Disabling Subtasks or Computer Use blocks new actions immediately, and agents receive current feature settings on their next turn without restarting.
...
- Keep new task names derived from their first prompt

## [0.2.18]

- Keep Child Task spinners reliable and show clear trailing icons for working, waiting, and failed Tasks
...
- Keep prompts and streaming replies in sync when sending from a client whose
  local transcript is missing older turns
...
- Include GPL, third-party notices, and matching source with every release
...
- Add standalone Shidou daemon and browser client
