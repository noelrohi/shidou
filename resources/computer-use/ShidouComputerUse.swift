...
private final class ComputerUseStatusItem {
    ...
    private func createItemIfNeeded() {
        ...
        item.length = 54
        item.button?.toolTip = "Shidou Computer Use"
        self.item = item
    }
    ...
    private func updateMenu() {
        ...
        let menu = NSMenu()
        let title = NSMenuItem(title: "Shidou Computer Use", action: nil, keyEquivalent: "")
...
enum HelperError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .missingPermission(let permission): "\(helperDisplayName) needs \(permission) access. Open Shidou Settings > Computer Use to grant it."
        case .unauthorizedClient(let reason): "Computer Use rejected a request that did not come from a trusted Shidou app: \(reason)"
        ...
        case .targetBlocked: "Shidou does not allow computer control of that app."
...
struct ShidouComputerUse {
            ...
            do {
                ...
                case "initialize":
                    result = [
                        ...
                        "serverInfo": ["name": "Shidou Computer Use", "version": "1.0.0"],
                    ]
                case "tools/list":
    ...
    static func serveMCPBridge() async throws {
        ...
        if let processDirectory = ProcessInfo.processInfo.environment["SHIDOU_COMPUTER_USE_PROCESS_DIRECTORY"],
...
private func readExactly(_ count: Int, from input: FileHandle) throws -> Data? {
    ...
    while data.count < count {
        ...
        if chunk.isEmpty {
            ...
            throw HelperError.ipc("the Shidou connection closed mid-message")
        }
        data.append(chunk)
    }
    return data
...
private func connectedChannel(at path: String) throws -> (FileHandle, pid_t) {
    ...
    do {
        ...
        guard getsockopt(descriptor, SOL_LOCAL, LOCAL_PEERPID, &peerPID, &peerPIDSize) == 0 else {
            throw HelperError.ipc("could not identify the Shidou process")
        }
...
private final class WindowCaptureSession {
    ...
    init(
        ...
        outputQueue = DispatchQueue(label: "dev.shidou.computer-use.capture.\(window.windowID)")
...
private func post(_ event: CGEvent, to processID: pid_t) {
    ...
    // input to a global event tap. PID-targeted delivery is what lets Shidou
