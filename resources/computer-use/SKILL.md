---
name: shidou-computer-use
description: Control local Mac apps through Shidou Computer Use for tasks that require reading or operating app UI. Prefer purpose-built connectors, APIs, or CLIs when available.
---

## shidou_js_repl + Shidou Computer Use

* Use the `js` tool from `shidou_js_repl` for all Computer Use actions.
* Do not use other technologies besides `shidou_js_repl` for computer interactions, unless specifically requested by the user (e.g. AppleScript, `osascript`, JXA, System Events, CGEvent synthesis).
...
Shidou exposes the native Computer Use runtime directly to QuickJS, but initializes `sky` lazily. Do not import `@oai/sky` or call the raw Computer Use helper.

Run this once per fresh `shidou_js_repl` session:
...
* It is usually not necessary to pause between performing an action and getting the updated app state. Shidou automatically waits up to about one second after a recent action before capturing the new state.

## Reading screenshots

Screenshot URLs are in `screenshot.url`, and Shidou returns them as base64-encoded `data:` URLs. To read a screenshot:
...
## Shidou target restrictions

Shidou Computer Use cannot control Shidou itself, Codex, ChatGPT, Sky, password managers, System Settings, login or security prompts, Keychain Access, or terminal apps. Do not try to bypass these restrictions.
