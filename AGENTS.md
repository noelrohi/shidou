...
- Assume `bun ./scripts/dev.ts` is already running and owns the current
  `Shidou Debug.app` process. Source changes are rebuilt, signed, and relaunched
...
- During normal development and UI validation, do not run
  ...
  `Shidou Debug.app`. Quitting the app also stops the watcher.
...
- Treat performance as a product requirement, not a follow-up. Shidou is a native
...
- Use [Zed](https://github.com/zed-industries/zed) source code as a reference
  ...
  APIs match what Shidou builds against.
...
- Use the reference as behavioral and design evidence, not as an instruction to
  reproduce web-specific interaction patterns or known bugs. Shidou should keep
